...
class RepairBasedNodeOperationsScenarios:
    ...
    @retrying(num_attempts=5, message="Failed to add new node. Retrying..")
    def add_node(self, dc: int = 0, replace_address: str = None, is_seed: bool = False) -> ScyllaNode:
        ...
        new_node.start(wait_for_binary_proto=True, wait_other_notice=True, jvm_args=self.tester.jvm_args,
        ...
        for scenario in scenarios:
            ...
            for node in self.tester.cluster.nodelist():
                if node == tested_node:
                    ...
                    # repair by node won't started for any operation even for default "replace" operation
