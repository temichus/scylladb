...
docker_cmd="docker run --detach=true \
    ...
    --privileged \
    ${DOCKER_IMAGE} bash -c 'pip3 install --user -e ${CCM_DIR} ; export PATH=\$PATH:\${HOME}/.local/bin ; cp -a /.ccm/repo* \${HOME}/.ccm/ ; bash -c \"${INSTALL_CASSANDRA}\"; python3 -m nose --nologcapture -v -s $*'"
