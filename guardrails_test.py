...
def create_ks_and_assert_warning(session, query, ks_name, key_warn_msg_words):
    ...
    ignore_the_result = ret.result()
    if len(key_warn_msg_words) > 0:
        assert len(ret.warnings) == 1, "Expected RF guardrail warning"
        assert all(word in ret.warnings[0].lower() for word in key_warn_msg_words), "Didn't match all required keywords"
...
class TestGuardrails(Tester):
    ...
    def test_default_rf(self):
        ...
        create_ks_and_assert_warning(session_dc1, query, ks_name,
                                     ["warn", "min", "replication", "factor", "3", "dc1", "2"])
    ...
    def test_all_rf_limits(self):
        ...
        cluster.set_configuration_options(values={
            ...
            'maximum_replication_factor_warn_threshold': 4,
            'maximum_replication_factor_fail_threshold': 5})
        ...
        ks_name = 'ks_e'
        create_ks_and_assert_warning(session, query % (ks_name, rf), ks_name,
                                     ["warn", "max", "replication", "factor", "4", "dc1", "5"])
