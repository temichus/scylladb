...
def create_self_signed_x509_certificate(test_path, cert_file='scylla.crt', key_file='scylla.key', ip_list=None, cname = None, ca_key = None, ca_cert = None, email=None):
    ...
    private_key = sign_key = rsa.generate_private_key(
    ...
    builder = builder.add_extension(
        x509.SubjectAlternativeName(altnames), critical=False
    )
    builder = builder.add_extension(
    ...
    certificate = builder.sign(
        private_key=sign_key, algorithm=hashes.SHA256(), backend=default_backend()
    )
...
def create_ca(test_path, cert_file='ca.crt', key_file='ca.key', cname = 'scylladb.com', valid = 365):
    ...
    root_cert = x509.CertificateBuilder().subject_name(
        ...
        datetime.datetime.utcnow() + datetime.timedelta(days=valid)
    ).add_extension(
    ...
    with open(file=key_file, mode="wb") as file:
        file.write(root_key.private_bytes(
            encoding=serialization.Encoding.PEM,
            format=serialization.PrivateFormat.TraditionalOpenSSL,
            encryption_algorithm=serialization.NoEncryption(),
        ))
