...
class TestSimpleCluster(Tester):
    ...
    def simple_consistency_level_validate(self, session, node, read_keys, read_cls_pass, read_cls_fail, read_cls_pass_all, write_keys, write_cls_pass, write_cls_fail, write_cls_pass_all):
        for cl in read_cls_pass:
            ...
            assert read_fail == 0 or not read_cls_pass_all, \
                f"Expected all reads to pass, pass {read_pass}, fail {read_fail}"
            assert read_fail > 0 or read_cls_pass_all, \
                f"Expected some reads to fail, pass {read_pass}, fail {read_fail}"
        ...
        for cl in write_cls_pass:
            ...
            assert write_fail == 0 or not write_cls_pass_all, \
                f"Expected all writes to pass, pass {write_pass}, fail {write_fail}"
            assert write_fail > 0 or write_cls_pass_all, \
                f"Expected some writes to fail, pass {write_pass}, fail {write_fail}"
    ...
    def test_simple_rf_1_consistency_level(self):
    ...
    def simple_query_validate(self, session, node, read_keys, query, read_cls_pass, read_cls_fail, read_cls_pass_all):
        for cl in read_cls_pass:
            ...
            try:
                ...
                errors = errors + [ex]
            assert read_fail == 0 or not read_cls_pass_all, \
                f"Expected all reads to pass, pass {read_pass}, fail {read_fail} {errors}"
            assert read_fail > 0 or read_cls_pass_all, \
                f"Expected some reads to fail, pass {read_pass}, fail {read_fail}"
