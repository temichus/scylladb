...
from tools.sslkeygen import create_self_signed_x509_certificate
...
class BaseAlternator(Tester):
    ...
    def create_snapshot(self, table_name: str, snapshot_folder: str, node: ScyllaNode) -> None:
        ...
        logger.debug(
            f"Creating local snapshot folder in following path '{snapshot_folder}' and moving all snapshot files to"
            f" this folder..")
    ...
    def _run_stress(self, table_name: str, node: ScyllaNode, target, num_of_item: int = NUM_OF_ITEMS,
        ...
        logger.debug(
            f"Start Alternator stress of {stress_thread.target_name}..\n Using parameters of: {stress_thread.kwargs}")
        stress_thread.start()
    ...
    def run_delete_insert_update_item_stress(self, table_name: str, node: ScyllaNode):
        ...
        def insert_item():
            ...
            if total_items % 25 == 0:
                logger.debug(
                    f"Updating '{len(update_items)}' existing items, creating '{len(new_items)}' new items and "
...
class BaseAlternatorStream(BaseAlternator, CDCInitializeHelper):
    @pytest.fixture(scope='function', autouse=True)
