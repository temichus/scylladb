...
def verify_durable_writes_keyspace(created_on_version, current_version, keyspace, session, table_name_prefix=""):
    ...
    for keyspace, is_durable in expected.items():
        ...
        assert is_durable == meta.durable_writes, \
...
def verify_compact_storage_composite_table(created_on_version, current_version, keyspace, session,
                                           table_name_prefix=""):
    table_name = _table_name_builder(table_name_prefix, "test_compact_storage_composite")
    meta = session.cluster.metadata.keyspaces[keyspace].tables[table_name]
...
def verify_nondefault_table_settings(created_on_version, current_version, keyspace, session, table_name_prefix=""):
    ...
    if '2.1' <= current_version < '3.0':
        ...
        assert '"sstable_compression":"org.apache.cassandra.io.compress.DeflateCompressor"' in \
...
def verify_uda(created_on_version, current_version, keyspace, session, table_name_prefix=""):
    ...
    assert function_name + "(int,int)" in session.cluster.metadata.keyspaces[keyspace].functions.keys()
    assert aggregate_name + "(int)" in session.cluster.metadata.keyspaces[keyspace].aggregates.keys()
...
def verify_udf(created_on_version, current_version, keyspace, session, table_name_prefix=""):
    if created_on_version < '2.2':
        return
    function_name = _table_name_builder(table_name_prefix, "test_udf")
    assert function_name + "(double)" in session.cluster.metadata.keyspaces[keyspace].functions.keys()
...
class TestSchemaMetadata(Tester):
    ...
    def test_creating_and_dropping_table_with_2ary_indexes(self):
        ...
        self.session.execute("drop table born_to_die")
        assert self._keyspace_meta().tables.get('born_to_die') is not None
        assert self._keyspace_meta().indexes.get('ix_born_to_die_name') is not None
        assert 0 == len(self._keyspace_meta().indexes)
