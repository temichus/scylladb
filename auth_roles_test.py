...
class TestAuthRoles(Tester):
    ...
    def test_creator_of_db_resource_granted_all_permissions(self):
        ...
        mike_permissions.extend(function_resource_creator_permissions(
            'mike', '<function ks.state_function_1(int, int)>'))
    ...
    def test_grant_revoke_permissions(self):
        ...
        assert_invalid(mike,
                       ...
                       re.escape("mike has no MODIFY permission on <table ks.cf> or any of its parents"),
                       Unauthorized)
    ...
    def test_filter_granted_permissions_by_resource_type(self):
        ...
        cassandra.execute(
            "CREATE FUNCTION ks.state_func(a int, b int) CALLED ON NULL INPUT RETURNS int LANGUAGE javascript AS 'a+b'")
    ...
    def test_grant_revoke_udf_permissions(self):
        ...
        cassandra.execute("CREATE ROLE mike WITH PASSWORD = '12345' AND LOGIN = true")
        cassandra.execute(
            "CREATE FUNCTION ks.plus_one ( input int ) CALLED ON NULL INPUT RETURNS int LANGUAGE javascript AS 'input + 1'")
        cassandra.execute(
            "CREATE FUNCTION ks.\"plusOne\" ( input int ) CALLED ON NULL INPUT RETURNS int LANGUAGE javascript AS 'input + 1'")
    ...
    def test_function_resource_hierarchy_permissions(self):
        ...
        cassandra.execute(
            "CREATE FUNCTION ks.func_one ( input int ) CALLED ON NULL INPUT RETURNS int LANGUAGE javascript AS 'input + 1'")
        cassandra.execute(
            "CREATE FUNCTION ks.func_two ( input int ) CALLED ON NULL INPUT RETURNS int LANGUAGE javascript AS 'input + 1'")
        ...
        assert_invalid(mike, select_two,
                       re.escape("User mike has no EXECUTE permission on <function ks.func_two(int)> or any of its parents"),
                       Unauthorized)
    ...
    def test_udf_permissions_validation(self):
        ...
        assert_invalid(mike, cql,
                       re.escape("User mike has no AUTHORIZE permission on <function ks.plus_one(int)> or any of its parents"),
                       Unauthorized)
        ...
        assert_invalid(mike, cql,
                       re.escape("User mike has no DROP permission on <function ks.plus_one(int)> or any of its parents"),
                       Unauthorized)
        ...
        assert_invalid(mike, cql,
                       re.escape("Unconfigured function ks.no_such_function(int,int)"),
                       InvalidRequest)
    ...
    def test_udf_with_overloads_permissions(self):
        ...
        cassandra.execute(
            "CREATE FUNCTION ks.plus_one ( input int ) CALLED ON NULL INPUT RETURNS int LANGUAGE javascript AS 'input + 1'")
        cassandra.execute(
            "CREATE FUNCTION ks.plus_one ( input double ) CALLED ON NULL INPUT RETURNS double LANGUAGE javascript AS 'input + 1'")
    ...
    def verify_udf_permissions(self, cql):
        ...
        assert_invalid(mike,
                       cql,
                       re.escape("User mike has no EXECUTE permission on <function ks.plus_one(int)> or any of its parents"),
                       Unauthorized)
    ...
    def test_disallow_grant_execute_on_non_function_resources(self):
        ...
        assert_invalid(cassandra, "GRANT EXECUTE ON TABLE ks.t1 TO mike",
                       re.escape("Resource <table ks.t1> does not support any of the requested permissions"),
                       SyntaxException)
        # Resource type RoleResource does not support any of the requested permissions
    ...
    def test_aggregate_function_permissions(self):
        ...
        assert_invalid(mike,
                       create_aggregate_cql,
                       re.escape("User mike has no EXECUTE permission on <function ks.final_function(int)> or any of its parents"),
                       Unauthorized)
