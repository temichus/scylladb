#!groovy
/*
   For gating we need to be able to run multiple builds at the same time, since in dtestPipeline.groovy we have disableConcurrentBuilds() configured, we had to duplicate the file.
   Also removed RUN_DTEST_HEAVY and RUN_DTEST_LONG code since they are not needed for gating
*/
def call(Map pipelineParams) {
    pipeline {
        parameters {
            ...
            string(name: 'INCLUDE_DTESTS', defaultValue: "${pipelineParams.get('INCLUDE_DTESTS', '-m next_gating')}" , description: """Specify which dtests to run. default for release:
