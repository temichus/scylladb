...
def checkAndTagAwsInstance (String runningUserID) {
	// TAG the spot instance with more tags
		wrap([$class: 'BuildUser']) {
			withCredentials([string(credentialsId: 'jenkins-releng-aws-secret-key-id', variable: 'AWS_ACCESS_KEY_ID'),
			string(credentialsId: 'jenkins-releng-aws-secret-access-key', variable: 'AWS_SECRET_ACCESS_KEY')]) {
            sh("""
