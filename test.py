...
def run_pytest(options: argparse.Namespace, run_id: int) -> tuple[int, list[SimpleNamespace]]:
    ...
    if options.list_tests:
        ...
        args.extend([
            ...
            "-rs",
            # test.py had a better mechanism to control the number of CPU and amount of memory allocated for each test
            # decrease the amount of thread to make tests more stable
            f'-n{int(options.jobs) - 3}',
            f'--tmpdir={temp_dir}',
