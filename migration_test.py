...
class MigrationTestBase(Tester):
    ...
    def migrate_sstable_with_variant_data_types(self):
        node1 = self.start_cluster_and_get_node1()
        query = 'CREATE COLUMNFAMILY ks.cf (aascii ascii, abigint bigint, ablob blob, aboolean boolean, adecimal decimal, adouble double, afloat float, ainet inet, aint int, atext text, atimestamp timestamp, atimeuuid timeuuid, auuid uuid, avarchar varchar, avarint varint, alist list<int>, amap map<int,int>, aset set<int>, PRIMARY KEY (aascii, abigint)) WITH comment=\'test cf\' AND read_repair_chance=0.000000'
        self.create_ks_and_cf(node1, None, None, False, query=query)
        node1.flush()
        self.load_migrated_tables(node1, 'variant_data_types')
        self.check_number_of_rows(node1, 3)
        result = self.get_all_rows_for_check(node1)
        import datetime
        import uuid
        ...
        self.assertEqual(str(result[0].ablob).encode('hex'), '0000000000000003', "check blob column" )
        ...
        self.assertEqual(round(result[0].afloat,2), 11.11, "check afloat column")
        self.assertEqual(result[0].ainet, '204.202.130.223', "check ainet column")
        self.assertEqual(result[0].aint, 17, "check inet column")
        self.assertEqual(result[0].atext, "text", "check text column")
        self.assertEqual(result[0].atimestamp, datetime.datetime(2016, 8, 30, 7, 1), "check timestamp column")
        self.assertEqual(result[0].atimeuuid, uuid.UUID('e23f450f-53a6-11e2-7f7f-7f7f7f7f7f7f'), "check timeuuid column")
        self.assertEqual(result[0].auuid, uuid.UUID('123e4567-e89b-12d3-a456-426655440000'), "check uuid column")
        self.assertEqual(result[0].avarchar, unicode("tzachvarchar"), "check varchar column")
        ...
        self.assertEqual(str(result[1].ablob).encode('hex'), '0000000000000003', "check blob column")
        ...
        self.assertEqual(round(result[1].afloat, 2), 11.11, "check afloat column")
        self.assertEqual(result[1].ainet, '204.202.130.223', "check ainet column")
        self.assertEqual(result[1].aint, 17, "check inet column")
        self.assertEqual(result[1].atext, "text", "check text column")
        self.assertEqual(result[1].atimestamp, datetime.datetime(2016, 8, 30, 7, 1), "check timestamp column")
        self.assertEqual(result[1].atimeuuid, uuid.UUID('e23f450f-53a6-11e2-7f7f-7f7f7f7f7f7f'),
                         "check timeuuid column")
        self.assertEqual(result[1].auuid, uuid.UUID('123e4567-e89b-12d3-a456-426655440000'), "check uuid column")
        self.assertEqual(result[1].avarchar, unicode("tzachvarchar"), "check varchar column")
        ...
        self.assertEqual(str(result[2].ablob).encode('hex'), '0000000000000003', "check blob column")
        ...
        self.assertEqual(round(result[2].afloat, 2), 11.11, "check afloat column")
        self.assertEqual(result[2].ainet, '204.202.130.223', "check ainet column")
        self.assertEqual(result[2].aint, 17, "check inet column")
        self.assertEqual(result[2].atext, "text", "check text column")
        self.assertEqual(result[2].atimestamp, datetime.datetime(2016, 8, 30, 7, 1), "check timestamp column")
        self.assertEqual(result[2].atimeuuid, uuid.UUID('e23f450f-53a6-11e2-7f7f-7f7f7f7f7f7f'),
                         "check timeuuid column")
        self.assertEqual(result[2].auuid, uuid.UUID('123e4567-e89b-12d3-a456-426655440000'), "check uuid column")
        self.assertEqual(result[2].avarchar, unicode("tzachvarchar"), "check varchar column")
